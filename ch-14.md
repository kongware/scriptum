...
The transformation from `Option` to `Task` works as expected. `Some` is mapped to the resolve handler of the continuation and `None` to the reject handler. However, there is no mapping the other way around. `Task` represents asynchronous computations whereas `Option` represents synchronous ones, that is, when the former starts working the latter is already finished.
