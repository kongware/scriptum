...
const numBetween = predAppend(Pred(numLte_(10))) (Pred(numGte_(5)));

numBetween.pred(5); // true
```
[run code](https://repl.it/repls/AggravatingAquamarineSystemadministrator)

A predicate is an unary function wrapped in a `Pred` value to distinguish it from other functions. Predicates are fixed in the type of their codomain, i.e. they always return a boolean. The `Monoid` instance let us compose simple predicates to build more complex ones. `Pred` adds type safety to our code, because we cannot use predicates in lieu of functions anymore. Moreover it makes our code more descriptive, because it makes predicates explicit.
