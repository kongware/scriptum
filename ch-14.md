...
Consequently for most monoids both computations yield different results. Considering this we can either manually supply both operations by adding a `prepend` function or we can define a type that flips the arguments:
...
```
Obviously, `Dual` adds a certain noise to the calling site. In the scriptum library I therefore prefer the manual approach and declare both `append` and `prepend` operations for each type.
