...
type empty<A> = () => A;
```
As you can see from the data type `Monoid` only operates on values, without distinguishing the context like `Functor`. Where monoids captures the notion of combining things functors captures the notion of context. We will see in later chapters in this course that many subclasses of `Functor` are just variations of monoids that are context aware.
...
Consequently it makes sense to define an operation with flipped arguments. We can achieve this either by manually supplying both operations or by declaring a type that reverses the arguments for us:
...
`length` has the above properties and is thus a valid monoid homomorphism.
...
```
`Parallel` represents asynchronous computations which are processed in parallel. Its monoid instance picks the result value of the faster one of two asynchronous computations. It represents the race monoid. The `Monoid` instance looks as follows:
...
Scriptum uses directory passing style and hence can define several monoid instances per type. Consequently we can spare type wrappers in many cases:
