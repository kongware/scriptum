...
type empty<A> = () => A;
```
As you can see from the data type `Monoid` only operates on values, without distinguishing the context like `Functor`. Where monoids captures the notion of combining things fucntors captures the notion of context. We will see in later chapters in this course that many subclasses of `Functor` are just variations of monoids that are context aware.
...
```
##### `taskPara` transformation

`taskPara` is the transformation function between `Task` and `Parallel`:

```javascript
const taskPara = tx =>
  Parallel((res, rej) => tx.task(x => res(x), rej));
```
but is it also a homomorphism between both monoids instances? In order to be a legit monoid homomorphism it would have ti preserve the monoidal structure:
...
So no, `taskPara` is not a monoid homomorphism but just a transformation between `Task` and `Parallel`
