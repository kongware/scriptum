...
There are some types that form only a semigroup but not a monoid. Howeverm we can easily promote such types to monoids using the  polymophic `monoidAppend` and `monoidEmpty` operations:
...
`monoidEmpty` plays the role of the missing identity element.
