...
A frequent scenario where immutability matters is gradual data acquisition. If you query external data, process it and query more external data depending on the results you often face the problem to combine this data in a structure, which is gradually growing. On first sight you can either mutate the given data structure or copy it each time you add information. However, a viable alternative is to create data structures sparately, which are euqal in shape:
...
```
With a couple of suitable combinators we can easily retrieve the relation between data components `[1, "a", true]`, `[2, "b", false]` and `[3, "c", false]`. This approach works for some scenarios but quickly becomes unmanagable as soon as the number of necessary mutations grows significantly.
