...
Race conditions exist in any language. In single threaded Javascript it is the event loop, which allows asynchronous computations. I use a short example to clarify the second point:
...
By the way, the example above entails a great lesson why mutations may be harmful. Maybe you have already wondered why `iarrToArr` is wrapped in an implicit thunk. Well, I treat `Array` as a mutable data type and hence destructively `push` values onto it. With `iarrToArr` values are pushed onto the accumulator `acc`. This is a side effect! Look what happens if I [remove the thunk](https://repl.it/repls/QuaintStableTests). I bet you have not anticipated this behavior - at leat I did not. So avoid side effects. Lesson learned.
