...
By the way, did you notice that I had to deal with the _non-idempotence_ error from the first example of this chapter? I wrapped `iarrToArr` in an additionaö lambda abstraction (see the underlined lines of code) to keep the operation idempotent. I had to take care of side effects and the errors they cause, because I treat arrays as a mutable data type. The crucial part is `arrSnoc`'s implementation `arrSnoc = x => xs => (xs.push(x), xs)`, which pushes elements onto the `Array`. See what happens when I [remove the additional lambda abstraction](https://repl.it/repls/QuaintStableTests).
