...
By the way `Hamt`, the persistent data structure used in this course is based on a hashed array mapped trie (HAMT). Besides `String`s it allows all sorts of types as keys, including `Object` types. It has a pretty straightforward API:
...
```
The principle to modify such a persistent data structure is always the same: A function takes a `Hamt` instance and at least a key of arbitrary type and returns a new `Hamt` with the modified key and shares all other keys with the previous instances.
...
`cons`/`snoc` are the immutable functional counterparts to `unshift`/`push` from the imperative world. It should be clearer now how the `props` argument of most `Hamt` functions is meant to be used. `hamtSet` expects two `props` arguments, because the set operation can either add a new key/value pair or modify an existing one.

You have probably seen the biggest downside of `Hamt` and persistent data structures in Javascript in general: They are not native and therefore reuqire a lot of transformations. `Hamt` is a tradeoff between performance gain through immutability and and performance loss through transforming data types. As a rule of thumb we can state that the benefits of persistent data structures outweigh their drawbacks for large amounts of data.

By the way, the example above entails a great lesson about the advantages of immutability. Maybe you have already wondered why `iarrToArr` is wrapped in a thunk. Well, I treat `Array` as a mutable data type and hence `push` values onto it. With `iarrToArr` values are pushed onto the accumulator `acc`. This is a side effect! Look what happens if I [remove the thunk](https://repl.it/repls/QuaintStableTests). I bet you have not anticipated this. At leat I did not. One more time: Avoid side effects!

Now we are prepared to reimplement the `Array` based and thus inefficient `take` function from the beginning of this chapter with a version based on `Iarray`:
