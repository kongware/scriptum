...
Typescript does not natively support Haskell-style implicit type classes, but simple name overloading. While there is a way to imitate the former, it leads to rather entangled and complex type definitions. For the sake of simplicity I decided to fall back to dictionary passing style, which basically means explicit type classes at the term level.
