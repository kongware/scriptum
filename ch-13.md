...
This is merely a very simplified description of a functor though. The concept does not stop at containers. It begins with them.
...
The structure context is obvious and is in general associated with functors. Effects, however, are the even more extensive category. We can think of functors as a concept that allows us to encode effects as values and reuse our ordinary pure functions to operate on these effect-values.
...
```
`len`, `head` and `reverse` all describe the structure of arrays, because they do not transform their values. This is the closest we can get to the essence of what constitutes the array context.
...
Typescript does support Haskell-style implicit type classes, but it does support simple name overloading. While there is a way to imitate the latter it leads to entangled and complex type definitions. Therefore I decided to resort to dictionary passing style, which basically means explicit type classes at the term level.
...
Now that we have settled on the structure of type dictionaries and how to apply them to functions let us declare a real world ad-hoc polymorphic function using dictionary passing style:
...
Here is a more complex example with several overloaded functions involved:
