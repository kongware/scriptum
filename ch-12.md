...
Type classes go beyond simple name overloading by giving the mapping from name to types itself a name. Now we can refer and, for instance, assign several overloaded names to such a named mapping, that is, we can put them into a relation. A type that has an instance of such a type class must respect this relation and implement all overloaded names. Additionally we can conceive some laws the overloaded functions within the type class must abide by and if our type system is expressive enough, it can enforce these laws.
