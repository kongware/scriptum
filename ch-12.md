...
Parametric polymorphism is a powerful but also somewhat limited form of polymorphism. The basic idea is that a function or value constructor must not know anything about its polymorphic arguments, because they can be of any type. Such arguments are expressed with type variables (e.g. `A` or `T`), which act like wildcards. When an algorithm does not no anything about an argument it cannot transform it or touch it in any way. All it can do is to arrange it within a structure or put it into an effectful context.
...
Compared to the previous example it goes the other way around. We start with a parametric polymorphic higher order function and pass it an overloaded function `arrEq(eq)`, which renders the type less polymorphic, namely ad-hoc polymorphic. You can think of such a partially applied function as one with a constraint: It must only be invoked with types that have an instance of the overloaded name `eq`.

Type classes go beyond simple name overloading by giving the mapping from name to types itself a name. Now we can reference and assign several overloaded names to such a named class, that is, we can put them into a relation. A type that has an instance of a type class must respect this relation and implement all overloaded names. Additionally we can conceive some laws the overloaded functions within the type class must abide by and if our type system is expressive enough, it can enforce these laws.

In this course we will stick to overloaded names, since type classes cannot be expressed with vanilla Typescript.
