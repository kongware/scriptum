...
A type constructor takes zero or more types and returns a new type. It lives at the type level of the language. Since Javascipt has no compile time there is no type level and no proper type constructors. The distinction is still useful since it allows a better understanding of type systems in general. A type constructor that takes no types is just an ordinary type itself. In the following example value constructors with increasing complexity along with their types are listed. Later in this chapter we will take a look at a data declaration that includes a proper type constructor and several value constructors.
...
The good thing about dictionary passing style is how simple it is. A type dictionary is just an additional argument containing one ore more functions and/or values. It is defined as the first formal parameter of a function and passed as an object to avoid a certain order:
...
I will show more examples with dictionary passing style during the introduction of common type classes in subsequent chapters of this course. Besides we will learn how to obtain mutli-parameter type classes and functional dependencies by utilizing them.
