...
Unfortunately, such higher order type constructors cannot be natively expressed in Typescript. There are rather complex workarounds available, but personally I think they establish too great an indirection. For this reason I will introduce a viable alternative in the second part of this course, which is engaged in typed functional programming.
...
Type class polymorphism goes beyond name overloading by additionally establishing relations between overloaded names. This way constrained ad-hoc polymorphic functions can be defined. Consider an ad-hoc polymorphic function that only accepts types that implement another overloaded function.

In this course I will realize ad-hoc polymorphism through type classes. Since Javascript does not support type classes at a language level, I resort to dictionary passing style. Type dictionaries are basically explicit type classes. They are less convenient to handle but equally expressive.

### Subtype polymorphism

Subtyping is the polymorphism you get when working with class hierarchies. It does not matter in the functional paradigm and is therefore only mentioned here.

### Row polymorphism

Row polymorphism enables record types to be polymorphic in their fields. Such a type accepts records with more fields than it has declared itself, provided these extra fields coincide for all involved recrods:
...
### Type classes through dictionary passing style

The good thing about dictionary passing style is how simple it is. A type dictionary is just an additional argument containing one ore more functions and/or values. It is defined as the first formal parameter of a function and passed as an object to avoid a specific order:
...
I will show more examples with dictionary passing style while introducing common type classes in subsequent chapters of this course. Apart from that we will learn how to obtain mutli-parameter type classes and functional dependencies by enhancing type dictionaries.
