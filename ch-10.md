...
The identity transducer does nothing else than behaving like a transducer. In order to do something useful a transducer must take additional arguments and do something useful with them. Here are the map and filter transducer:
...
Although two distinct transducers are involved we only need to traverse the array once. The composition of transducers enable loop fusion. And since `map` expects a normal pure function as its first argument it can be a composed function as well. In a later chapter of this course we will learn how to compose predicate functions as well, as they are expected from the `filter` transducer.
