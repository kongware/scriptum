...
Another trait of transducers is their ability to abstract from their data source. A transducer only deals with a single value at a time. It does neither know anything about the underlying structure nor in what order values are pulled out of it. As a consequence we can apply every transducer to every data structure that is at least `Foldable`, i.e. that has a corresponding fold function:
