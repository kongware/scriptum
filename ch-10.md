...
Although two distinct transducers are involved we only need to traverse the array once. Instead of passing each array element to one transducer after another they are passed to the transducer composition, which can include an arbitrary number of transducers. The composition fuses the loops so to speak. But how does this exactly work? When we look at `comp` we see that `f` and `g` are meant to be unary functions. However, the partially applied `map(sqrLen)` and `filter(isEven)` both expect three further arguments. How does this fit together?
...
const comp = f => g => x => f(g(x));     // apply partially applied map
  ...
  // reduce applications until no further reduction is possible
...
* `x` of `comp` is not an arbitrary value but a function, namely `arrSnoc_`
...
Next the fold function applies the resulting function to the accumulator `[]` and to each element contained in the array one at a time.
Since `map` and `filter` are wired through their `append` functions first `map` is processed with each element and then `filter`. The following conclusions can be drawn:

* the array is only traversed once
* transducer composition goes from left to right.
