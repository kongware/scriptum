...
const infix4l = (v, f, w, g, x, h, y, i, z) =>
  i(h(g(f(v) (w)) (x)) (y)) (z);
...
`infix` et al. mimic infix notation with prefix functions. This is again not that elegant but effective. There is also an overloaded `infixn` and a simple binary `infix` combinator, of course.
