...
Lazy evaluation also means to evaluate sub-expressions just enough, that is to suspend evaluation as early as possible. If the outermost level of an expression is a lambda abstraction or a constructor, it need not be further reduced. Such an expression is in WHNF, that is, it may contain unevaluated sub-expressions, which are referred to as thunks. Taking the above example the add function call is in WHNF:
...
Lazy evaluation would be very inefficient if it had to evaluate the same sub-expression each time it occurs in a function body. For that reason the result of once evaluated sub-expressions is stored and shared within the same scope:
