...
logUserInput(x)
```
in a lazy setting, then you clearly cannot just use functions with side effects without any precaution. So does a lazy language do not have any order at all? Of course it has and it is mainly based on the right or left associativity of the involved operators.
