## Functional Jargon and Developer Experience
...
Composition lets us decompose large tasks into small functions, each focused on a single responsibility, which then can be recomposed in various combinations. This approach leads to both descriptive and reusable code.

#### Local reasoning

We have learned about referential transparency, which is a rather technical and abstract term. What does RT ultimatively give us? Well, it enables us to reason locally about our code. Local reasoning means we can think about an expression, which is deeply burried in our program's structure without being concerned about the context. In the absence of side effects we can indeed forget about the context and focus on the portion of code we are interested in. This is an incredible strong idea and it scales with the size of your program. Along with the general idea of composition it represents the main benefit you can experience as a functional programmer.
...
```
and goes on with more complex laws associatived to certain patterns and data types, like identity, composition or the homomorphism law. Equational reasoning allows various transformation of code and the result just works as expected. It requires a lot of experience so hang in there.

#### Explicit effects but no side effects

The functional paradigm forces you to make computational effects explicit by transforming them into first class values and typing them properly. A computation that may not yield a result explicitly says so. A computation that carries around some state explicitly says so. Having all theses special values flying around throughout your application can be cumbersome sometimes, but they also discipline you to consider all edge cases and avoid bugs.
