...
There are two fundamental operations you can apply to functions, application (e.g. `f => x => f(x)`) and composition (e.g. `f => g => x => f(g(x))`). Function application is the elimination rule of the function type and function composition is its dual. Loosely speaking you can think of it as the dynamic introduction rule of the function type. The actual introduction rule of the function type is function declaration, of course, which takes place at compile time and is thus static.

### The functional programming experience

#### Composition

Composition lets us decompose large problems into small functions, which then can be recomposed to solve the orignal problem. This approach has far-reaching consequences on the coding style. Since small functions tend to be focused on a single concern, we gain a high degree of code reuse. As soon as you compose and apply lots of small functions with meaningful names you gain pretty descriptive code, where the underlying algorithms are abstracted away.

#### Local reasoning

We have already learned about referential transparency, which is a rather technical term. What does RT ultimately give us? Well, it enables local reasoning. This means we can think about an expression, which is deeply involved in our program's structure, without having to be concerned about the context. With the absence of side effects we can focus on the piece of code we are interested in. There are a few exceptions like free variables from closures but the idea holds in general. As a matter of fact local reasoning is an incredible strong concept which scales with the size of your program. Along with composition it is one of the corner stones of the functional paradigm.
...
```
and goes on with more complex laws associated to certain patterns and data types, like identity, composition or the homomorphism law. Equational reasoning allows various transformation of code and the result just works as expected. Such transformations require a lot of experience though, so hang in there.

#### Effects as values

Functional programming can be adequately described as programming with values. We have already learned that functions are values. In the same sense effects are values, because the results of effectful computations are assigned to a container-like type that represents both the value and the effect level. Being reified as values effects can be passed around like first class entities. But it also means that we cannot freely access the containing result value of an effectful computation anymore, unless we know how to unwrap it.

#### Recurring general patterns

There are a couple of ad-hoc but rather general and lawful patterns in functional programming, which are applicable to many data types and hence are ubiquitous in a functional codebase. With a little practice you are able to recognize these patterns in different scenarios. Imagine how predictable and easy to reason about such code becomes.

#### The inhibition threshold

For novices functional programming is frequently hard and counterintuitive in the beginning and requires a high level of frustration tollerance. However, if they get through this phase they quickly reach high productivity in creating recilient, high-quality code. So hang in there!
