...
Although it only marginally fits to the topic at hand I want to point out to type wrappers. Instead of using native types you should as often as possible wrap them in more meaningful singleton types to create additional meaning, because this way you can significantly improve the readability of your code. You could object that we can use descriptive names instead. Keep in mind, though that with functional programming there are often large compositions lacking intermediate name bindings:
