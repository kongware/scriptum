...
Type wrappers are used to create values of built-in types with additional semantics at the type level. While the handling of such values is a bit more laborious it helps to emphasize the intention of your code:
...
const x = DegreeCelsius(20),
  ...
  z = DollarAmount(1.59);
```
You should do this on a regular basis. Please keep also in mind that we often manage large compositions in functional programming which lack descriptive intermediate values. The more important it is to have meaningful runtime types during debugging.
