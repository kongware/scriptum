...
Instead of assigning intermediate results to variables a functional programmer composes functions - not only unary pure ones but all kinds if functions. Such compositions have a strict order caused by data dependencies between the involved functions.

### Algebraic data types

Algebraic data types are the only means necessary in the functional paradigm to construct all kinds of composite data structures. They are composable and may have recursive definitions. The following sections cover the most important ADT classes.

#### Sum types
***

A sum type, also known as tagged union, can have alternative shapes called variants, where only a single variant can exist simultaneously. Scriptum supplies the `union` auxiliary function to facilitate the construction of tagged unions in Javascript:
...
```
The `Bool` tagged union type from the above example can either be `True` or `False`. Please note that the `Bool` property is required, because we will use scriptum along with  Typescript in the second part of this course. Such a property allows Typescript's strutural type system to distinguish different union types with an accidentally identical structure.
