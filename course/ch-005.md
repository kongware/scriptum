...
### Gaining more control over the evaluation time
...
const comp = f => g => x =>
  ...
  ^^ nullary function
```
Nullary functions are also referred to as thunks in Javascript. The type is infectious, because consumers of nullary functions need to apply them, even though there is no argument. Can we render lazy evaluated code more natural?

#### Implicit thunks

We can indeed, provided we are willing to intercept property accesses and functions calls with the `Proxy` interface:
