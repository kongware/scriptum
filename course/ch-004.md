...
While associativity renders order irrelevant at the operation level, commutativity relaxes it on the operand level by allowing to swap them:

```
x op y = y op x
f(x) (y) = f(y) (x)
```
The number of useful algebraic structures that respect the commutative property is much smaller than those who are associative. While addition and multiplication is commutative, subtraction, division and appending two arrays is not.
