...
Functonal programming is based on a number of mathematical structures, properties and laws. Its strong foundation on math is really the most fundemantal difference to other paradigms.
...
Here is a list of basic algebraic properties that are required by various algebraic structures:
...
There are other more specific laws that only apply to a single algebra. Here are examples for the applicative functor:
...
* distinction between operator associativity and associative property
* associativity allows evaluation in parallel, because order of operations doesn't matter
* associativity only matters if a function/operator has the same type in its domain/codomain
* commutativity relaxes the order requirement even further by allowing no-order in connection with operands
* idemotency is regularly gained by only relying on pure functions
