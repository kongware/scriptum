...
f(f(x) (y)) (z) = f(x) (f(y) (y))
```
This is an extremely useful feature of the functional paradigm, because it enables the evaluation of operations out of their lexical order and thus in parallel, for instance.

The associative property must not be confused with operator associativity, though. Function composition is associative. Please note that since Javascript does not have a composition operator I demonstrate the property using a function:
...
```
The composition operator in Haskell, for instance, is left associative, that is, `f ∘ g ∘ h` is parsed as `(f ∘ g) ∘ h`.
