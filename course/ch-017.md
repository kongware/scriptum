...
The `Parallel` based computation lifts the pure `add` function into an asynchronous context by composing `map` with `ap`. The `Task` based computation on the other hand uses a nested closure instead of composition. There is no separate function to be lifted, but the nested computation itself forms the monadic action. 
