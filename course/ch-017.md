...
With monads the next effect may depend on the previous value. But what does this exactly mean for a single type class instance? Well, we cannot say in general, because it means quite different things for different monads. It is the hard part of the concept.

In a previous example of this chapter we have already seen what the value/effect dependecy means for the array instance, namely arrays of different shape. Since array is a linear data type this means arrays of different length:
...
The lenght of the above arrays is non-deterministic, because it depends on values to be determined at runtime. We cannot achieve the same effect with the `Applicative` instance. This aspect of a non-deterministic computation through arrays is essentially monadic.
