...
Once a computation yields a monad you cannot escape its algebraic structure anymnore. This corresponds to the functor and applicative functor behavior. If you want to apply a function to a value wrapped in a monad you need to promote this function to an action of matching type and apply it to the respective `chain` operation. You can write a function that unwraps the monad, of course, but such a function is beyond the monadic API and hence only works for the specific monad it is implemented for. So why is an unwrapping operation not part of the API? Because for some monads there does not exist such an operation. Such an operation would render monads less general.
...
* the programmable semicolon metaphor
