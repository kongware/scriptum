...
Monads do not handle I/O or other side effects. A pure language needs a magical type that contains impurity and provides means to connect the pure program to the real world. Fortunately, this magical I/O type forms a monad, which allows us to compose I/O operations in a principled monadic way. Now you could argue that applicative functor does this already. However, this only holds for associative operations the fewest I/O related ones belong to. Monadic sequences determine order, because the next computation may depend on the previous result. Hence monad is a suitable abstraction for such compositions with side effects.
