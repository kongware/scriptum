...
This also means that you should always attempt to use applicatives first. Only if you cannot express something with this algebraic structure you should resort to the extra power of monads, because such power always entails its very own drawbacks. During this chapter we will encounter some of them.
...
Every monad instance has to comply with the following laws:

```
≡        denotes equivalence
fm/gm/hm denotes a monadic action
...
If you have an imperative background I find the programmable semicolon metaphor quite helpful. In imperative programming we use statements, which are completely decoupled from each other. In order to use the result from a previous statement we have to rely on intermediate values. Every statement may include various effects. This it hard to tell, because they are implicit. Monads allow to compose such effectful statements in a principled fashion. Since statements are often concluded with a semicolon, you can think of a monad as a programmable and composable semicolon.

### Link between monads, I/O and side effects

Monads are no means to render I/O operations and thus on of the major classes of side effects pure. In a purely functional language you need a magical `IO` type to maintain purity, which connects your pure program to the real world. So what part do monads play in this context? The answer depends on the evaluation strategy of the language. If it pursues lazy evaluation there is no predetermined order expressions are evaluated in, so you need composition to enforce order. If you try to compose functions that perform I/O operations you will ultimately end up with a monad-like composition structure, that is, in a lazy setting monads are the only way to perform I/O - at least more than a single operation. With an eagerly evaluated language on the other hand you can cheat and perform I/O whenever you want, because the evaluation order is determined at compile time.

While monads are not a prerequisite for programming with side effects in Javascript, they are still very useful. Usually we distinguish two types each for synchronous (`Lazy`) and asynchronous (`Task` and `Parallel`) computations. We will take a closer look at these types in following chapters of this course.

### Monads do not compose

Unfortunately, despite all its expressiveness the monad algebra is no panacea. Its crucial capability of conflating term and effect level is also its weak spot, because it keeps monads of different type from being composable in a general manner. Here is a schematic representation of two composed monads `M` and `N`:

```javascript
// mmx has type M<N<A>>

chainM(mmx) (mx => // M layer
  chainN(mx) (x => // N layer
    ...));
```
The problem is that we cannot join the `M` and `N` effect layers in general. I will not demonstrate this property using an example at this point, because this chapter is already detailed enough. You just have to believe me. Breaking composition is a huge drawback, because with the functional paradigm everything is about composition. There are various competing approaches to tackle this problem. It is a field of active research and highly exciting. We will cover the most mature approaches in subsequent chapters of this course.
