...
// result of the action used as an ordinary value
arrAppend(foo(1) (2)) (foo(3) (4)); // [1, 2, 3, 4]

// result of the action used as a non-deterministic computation
arrAp(arrMap(foo) ([1, 2])) ([3, 4]); [[1, 3], [1, 4], [2, 3], [2, 4]]
```
[run code](https://repl.it/@scriptum/TubbyUnconsciousVolume)

Obviously we can use an action like a normal function and it behaves like expected. However, if we use it as part of an applicative computation, it behaves differently. The return value does not represent an ordinary value anymore, but a computation - a non-deterministic computation in the example above. This is 
