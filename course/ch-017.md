...
### Monads do not render I/O pure

Monads do not render computations that include I/O pure. This is a common misunderstanding. What makes `IO` pure is a special type. In Haskell it is called `IO`. In Javascript it is often called `Lazy` for synchronous and `Task`/`Parallel` for asynchronous computations. Monads merely come in handy as soon as you want to compose two `IO` computations. You can also combine them manually, but the monad interface greatly facilitates the operation. We will take a closer look into `Lazy`, `Task`, `Parallel` and the impure `main` function, which is also known as the runtime, in following chapters of this course.

### Monads do not compose

Unfortunately, despite all the expressivness the monad algebraic structure is no panacea. Its crucial capability of conflating term and effect level is also their weak spot, because it keeps them from being composable, at least in general. Here is a schematic representation of two composed monads `M` and `N` of different type:
...
M<A>
  N<A>
    M<A>
      N<A>
```
The problem is that we cannot join the `M` and `N` effect layers, at least not in general. This is a huge drawback, because with the functional paradigm everything is about composing. There are various competing approaches to tackle the problem. We will cover each and every of it in subsequent chapters of this course.
