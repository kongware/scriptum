...
### Value/effect dependency and effect sequencing

The special trait of monads is their ability to perform the next effect depending on the previous value. While this dependency is only optional, it presupposes that the next effect must depend on the previous one. You cannot have one without the other. Let us illustrate this in a more schematic manner. Given is an effectul computation `F<A>` where `A` is the result value of a previous effectful computation. It applies:
...
`Task` runs its effect in parallel whereas `Parallel` runs it in parallel. `Task` implements `Monad` and thus also `Applicative`. `Parallel` on the other hand only implements th e latter, because a monad cannot run in parallel by design. You might wonder why I did declare `Parallel` in the first place instead of implementing `Task`'s `Applicative` instance with in parallel semantics. Having a monadic type whose `Applicative` instance has different semantics is considered bad practice. This is only a convention but a useful one and you should adhere to it.

The `Parallel` based computation lifts the pure `add` function into an asynchronous context by composing `map` with `ap`. The `Task` based computation on the other hand forms a nested closure. There is no distinct function to be lifted but the nested computation itself forms the monadic action. The attentive reader may have already noticed that a monad is not necessary for the above task. I just chose addition as the underlying operation for both computations in order to demonstrate their differences.

While monadic conmputations usually run in sequence, there are a few exceptions. It might be a simplification but I think the notion of monads as effect sequencing operators is still helpful. Just keep in mind that the order effects are performed in is not always deterministic.
