...
The special trait of monads is their ability to perform the next effect depending on the previous value. That means while the next effect may depend on the previous value it must depend on the previous value. Given are two effectul computations `F<A>` and `F<B>`. It applies:
...
We can demonstrate this limitation with asynchronous computations that are run in parallel and in sequence:
...
As already mentioned `Parallel` has only an applicative instance, whereas `Task` has both, applicative and monad instances. We could have delcared a single type with a parallel applicative and a sequential monad instance, but such an approch is considered harmful, because both instances should have the same semantics.

`Parallel` lifts the pure function `add` into an asynchronous context. `Task` on the other hand applies an action with two monadic values. The action is not a distinct function but the entire nested computation itself. The attentive reader may have already noticed that a monad is not necessary for the exemplary computation. I use the same underlying operation (addition) anyway, to contrast the parallel versus sequential effect performance. The monadic version cannot run the addition in parallel, because the next computation has the potential to depend on the previous result.

Most monadic conmputations run in sequence and for that reason monads are sometimes called effect sequencing operators. Unfortunately, this is a simplification and not true for all monads. The exception confirms the rule though.
