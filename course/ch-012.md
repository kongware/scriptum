...
Staically bound name overloading is less verbose but Typescript does not support it.

There is a more powerful approach to achieve ad-hoc polymorphism, which is called bounded parametric polymorphism or simply type classes. Type classes go beyond name overloading by giving the mapping from a name to several types itself a name. Now we can refer to this mapping and, for instance, subsume several overloaded names under this name or assign algebraic laws to it. We can also define type class hierarchies to avoid repitition. Type classes have turned out to be the superior way to encode ad-hoc polymorphism.

In this course we will use a technique called dictionary passing style, because it is far more beginner frinedly and can be easier expressed with Typescript. While the dictionary passing style is even more expressive than type classes it comes at the price of making code more verbose. Type dictionary act like explicit type classes.
...
If we abstract further and assign a type to each type we get a system that is referred to as kind system. A kind is simply denoted by an asterisk `*`. Each monomorphic type has kind `*`. An unary type constructor has kind `* => *` and a binary one has kind `* => * => *`. The number of asterisks corresponds to the number of type variables in the original type - it is that easy. But how can we give higher-order type constructors a proper kind? We put the type costructor argument into parentheses. `(* => *) => * => *` is read as: A type constructor that takes another type constructor and a type and returns a new type.
