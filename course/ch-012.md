...
Functional progamming is type directed programming, hence we need to acquire some type theoretical background, even though the first part of this course relies on untyped code. Please note that I will use Typescript notation to annotate types.
...
A monomorphic type or in short a type is like a constant at the value level. It is complete and can no longer be modified:
...
I left out the type for `map` and `len`, because their types are not monomorphic but polymorphic. We will learn more about polymorphic types in a subsequent section of this chapter.
