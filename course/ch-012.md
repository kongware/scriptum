...
map(len) (["f", "fo", "foo"]) // <string, number>(f: (x: string) => number) => (xs: string[]) => number[]
```
Both functons have polymorphic types as well but we are only interested in the array type constructor inside. `Array<A>` takes a single type `A` and returns a new type. `A` is a type variable that serves as a placeholder for a type yet to be provided. In the example above `string` is passed to the type constructor, which in turn yields `string[]` as new monomorphic type. For `map` the array type constructor is applied twice. At first the yielded type is determined by the type of `f`'s domain (`string`) and for the second time by `f`'s codomain (`number`).
