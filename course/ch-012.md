...
If we abstract further and assign a type to each type we get a system that is referred to as kind system. A kind is simply denoted by an asterisk `*`. Each monomorphic type has kind `*`. An unary type constructor has kind `* => *` and a binary one has kind `* => * => *`. The number of asterisks corresponds to the number of type variables in the original type - it is that easy. But how can we give higher-order type constructors a proper kind? We just put the type constructor argument into parentheses. `(* => *) => * => *` is read as: An unary type constructor that takes another unary type constructor and a type and returns a new type.
