...
If a function must not know anything about its polymorphic type parameters, the operations it can apply to these arguments at the term level are rather limited. We can deminish this limitation by imposing constraints on polymorphic parameters. A type constriant is a value or function that specifies a certain behavior of a type. Constraints narrow the scope as to what the type can be used but allow more useful operations on the term level. We trade the generality at the type level for generality at the term level.

A function with a constraint on its polymorphic parameter takes additional values and/or functions, as soon as it knows which argument type it has to deal with. Under the hood they are just passed as additional implicit arguments. The function can now use these arguments to do something useful with its corresponding polymorphic argument. Consequently the function can only deal with those types that implement the type constraint, i.e. supply the necessary values and functions. The resolution of types and corresponding constraints happens during compile time and is kind of ad-hoc. This is the reason why type classes are classified as a form of adhoc polymorphism.
