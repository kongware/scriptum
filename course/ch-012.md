...
If a function must not know anything about its polymorphic type parameters, the operations it can apply to the corresponding arguments at the term level are rather limited. We can deminish this limitation by imposing constraints on polymorphic parameters. A type constriant is one or more value and/or function that specifies a certain behavior of a type. Constraints narrow the scope as to what the type can be used but allow more useful operations on the term level. We trade the generality at the type level for generality at the term level.

A bounded parametric polymorphic function is basically just a function that takes one or more additional implicit arguments for its polymorphic type parameter. These implicit  arguments add certain behavior to the function no matter what type the polymorphic type parameter specializes to. This requires that the spezialized type has implemented the demanded behavior, of course. Since polymorphic type parameters are implicit we need a resolution mechanism to actually pass values/functions to a bounded parametric polymorphic function. This binding is kind of ad-hoc and the reason why bounded parametric polymorphism is often classfied as a form of ad-hoc polymorphism.

* name overloading where the mapping from names to types itself gets a name
* this allows grouping overloaded names
* this allows assigning laws to such mappings
* this allows to relate such mappsings to each other and form mapping hierarchies
...
### The type-constraint/applicability correlation

**[Editor's note: this section needs edit]**

Parametricity has a somewhat strange effect on the applicability of polymorphic functions at the term level: The more constraints the type of a polymorphic functions has the more applicable the function gets. This is counterintuitive. It should rather be the other way around. In order to understand this strange correlation we must look at the type and term level separately. Parametricity only applies to the type level and enforces that a function does not know anything about its polymorphic arguments. Type constraints interfere with parametricity, because they tell the function how a constrained polymorphic argument behaves under certain circumstances. Consequently, the more type constraints a function has at the type level the more it knows about its polymorphic arguments and the more useful behavior it can exhibit at the term level.
