...
There is a more powerful approach to achieve ad-hoc polymorphism, which is called bounded parametric polymorphism or simply type classes. Type classes go beyond name overloading by giving the mapping from a name to several types itself a name. Now we can refer to this mapping and, for instance, subsume several overloaded names under or assign algebraic laws to it. We can also define type class hierarchies to avoid repition. Type classes turn out to be the superior way to encode ad-hoc polymorphism.

In this course we will use a technique called dictionary passing style, because it is far more beginner frinedly. While dictionary passing style even more expressive as type classes it comes at the price of making code more verbose. Type dictionary act like explicit type classes.
