...
If a function must not know anything about its polymorphic type parameters, the operations it can use the corresponding arguments with at the term level are rather limited. We can decrease this restriction by imposing constraints on these parameters. A type constraint is a value/function that specifies a certain behavior of an otherwise polymorphic type. Constraints narrow the scope as to what the type can be used but allow more meainingful operations on the term level. We trade the generality at the type level for generality at the term level.

A bounded parametric polymorphic function is basically just a function that takes one or more additional implicit arguments, which are demanded by its polymorphic type parameters. These implicit arguments extend the behavior of the function for a certain aspect. Consequently, a polymorphic type parameter can only specialize to a type that implements the required behavior. Since polymorphic type parameters are implicit we also need a resolution mechanism to actually pass the additional behavior in form of values/functions to polymorphic functions. This binding is kind of ad-hoc and the reason why bounded parametric polymorphism is often subsumed under ad-hoc polymorphism.
