...
* in OOP also called type constraints on generics
* function needs some knowledge about its polymorphic arguments
* type classes are paremtric polymorphism under the hood
* the resolution of bounds (type classes) is implicit and kinda ad-hoc
* narrows the scope as to what the type can be used but allows more useful operations on the term level
