...
`partial` is a sensible implementation of this imperative idiom. It allows deferring the evaluation of a multi argument function by calling it only with a subset of its arguments. The missing arguments are provided with the subsequent call. Currying takes place when a function is declared. It has nothing to do with its application. You cannot partially apply a curried function, because it always expects a single argument. However, it is convenient to say a curried function is partially applied when it returns another function insteaf of its "final result":
...
Variadic functions are non-deterministic in the number of their arguments, which can range from none, one to many. There are two classes of variadic functions to be distinguished:
...
Curried functions applied to function composition or other higher order combinators often lead to point-free style code. Point-free means to omit function arguments. Here is a simple example:
...
Point-free style is rather a side effect than something we deliberately aspire to. It happens as soon as we compose curried functions or apply them to higher order functions.
