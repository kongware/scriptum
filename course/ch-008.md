...
fib(10); // 55
```
Please not that there are catamorphisms for many data types, not only for natural numbers. Unfortunately, this is still not the end of the story. Strictly speaking primitive recursion corresponds to a paramorphism not to catamorphism. The former is a more convenient form of the latter, but theoretically equally expressive. A paramorphism has access to the intermediate state of the value to be folded, i.e. the partially folded value. This feature allows much more natural implementations for some recursive algorithms:
...
const fib = natPara([0, 1])
  (([x, y]) => _ => [y, x + y]); // no improvement
