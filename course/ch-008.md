...
As you can see these algorithms differ only in two areas at the same laxical positions. If we abstract from the boilerplate, the underlying structure is revealed, which is called a catamorphism in functional programming. You can think of a catamorphism as a slightly generalized fold. Please not that there are catamorphisms for many data types, not only for natural numbers.

Unfortunately, this is still a simplified description of primitive recursion. Strictly speaking the concept corresponds not to catamorphisms but paramorphisms, which are a more convenient form of the former. A paramorphism has access to the intermediate state of the value to be folded, i.e. the partially folded value. We will learn more about theses and other recursion schemes in a subsequent chapter of this course.
***
