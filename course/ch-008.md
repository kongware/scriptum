...
A recursive algorithm consists of one or more recursive steps and one or more base cases. A base case is the simplest, smallest instance of a data chunk that cannot be decomposed any further. A recursive step decomposes a larger instance of a data chunk into one or simpler or smaller instances. On the way forward to the base case either the function call stack or an accumulator is built up, which is unwind when the base case is reached. On the way back the intermediate values are recomposed to produce the solution to the original problem.
...
Since body recursion depends on the function call stack the problem size that can be handled is limited to the available stack size. In contrast to this tail recursion can share a single stack frame throughout the whole computation, because the accumulator takes over the role of the stack frame, provided the language pursues tail call elimination. Technically tail recursion is the functional equivalent of imperative loops. Body recursion however, requires a loop along with a custom call stack structure.
...
Primitive (or natural) recursion is a form of recursion where at each iteration the original data chunk is decomposed by the smallest or simplest possible instances of that chunk. This limitation ensures that every recursive algorithms terminates, because it is guaranteed to hit the base case at some point:
...
n => n - 1;
```
Both operations will ultimately reach the base case, which is the empty array or zero in the above instances. Here are a primitive recursive arithmetic functions that operate on natural numbers:
...
As you can see these algorithms differ only in two areas at the same lexical positions. If we abstract from the boilerplate, the underlying structure is revealed, which is called a catamorphism in functional programming. You can think of a catamorphism as a slightly generalized fold:
...
This is bare corecursion in its original form, because we work directly within the literal of the given type. A more abstract encoding but still corecursion can be defined using the `iterate` function:
