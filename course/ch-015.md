...
**[Editor's note: This chapter needs critical editing]**

If we have two values we can either accumulate (e.g. `[2, 3]`), aggregating (e.g. `2 + 3`) or pick one of them (e.g. `[2, 3] [0]`). All these methods are notions of the same broader concept, because they share certain properties we are going to examine in this chapter. With the `Monoid` type class we can write code that is agnostic about the method we will actually use.
