...
Monoids capture the notion of accumulating (e.g. [2, 3]), aggregating (e.g. 2 + 3) and picking values (e.g. 1st of [2, 3]), which are different notions of a broader concept: combining. Accumulating does not lose information, whereas aggregating and picking do. With the `Monoid` type class we can write code that is agnostic about the method we will actually use.
