...
Please note that the transformer stack we are going to build in this section is not particularly useful. I want you to solely focus on the mechanics of the composition and not on the issue whether the monad transformer stack exhibits a useful semantics.

Let us start with a simple composition where `OptionT` is the monad transformer and `Array` the base monad. It is tempting to simply use `Option`'s `optChain` in order to construct `OptionT`'s one, but unfortunately this is not allowed. Implementing a monad transformer means to construct brand new  `chain` and `of` functions that are specific to this transformer:
