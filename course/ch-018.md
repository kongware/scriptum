...
Let us start with a simple version that we already implemented by hand at the beginning of this chapter. `OptionT` is the monad transformer and `Array` the base monad. It is tempting to simply use `Option`'s chain in order to construct `OptionT`'s one, but unfortunately this is not allowed. Implementing a monad transformer means to construct brand new  `chain` and `of` functions that are specific to the transformer.
...
* add asynchronicity to `OptionT`
* then add asynch error handling
