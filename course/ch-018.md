...
The monad passed to the higher order one is called the base monad. If we compose two monads of different type we get back a nested monad, where the base monad becomes the inner and the higher order monad becomes the outer one. This is a bit counterintuitive:
