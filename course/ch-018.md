...
const randNum = () =>
  Some(Math.round(Math.random() * 10));

// MAIN

const mmx = [randNum(), randNum(), randNum()],
  mmy = [randNum(), randNum(), randNum()];
...
main; // array of random Some(number) or None
```
[run cdoe](https://repl.it/@scriptum/PaleStrictTraining)

The example above exhibits two interleaved effects. The first effect consists in a non-deterministic length of the array and the second one in `None` values that encode the division-by-zero error.
...
Next we flip the effects to prove that effect composition is not commutative:
