...
* difference between monads where the effect is tied to the value or the effect is deferred

#### `OptionT`/`EffT`/`Array` monad
...
Let us start with a simple composition where `OptionT` is the monad transformer and `Eff` the base monad. It is tempting to simply use `Option`'s `optChain` in order to construct `OptionT`'s one, but unfortunately this is not possible. A monad transformer represents a brand new type and implementing it means to construct `chain` and `of` functions that are specific to this type.

`Eff` encodes computations that have an synchronous effect. Theonly reasonable thing we can with such computations is to defer their execution as long as possible. We will learn more about effect deferral in a subsequent chapter of this course. Here is our first monad transformer stack:
...
const effOptChain = optChainT(
  {chain: effChain, of: effOf});

const effOptOf = optOfT(effOf);
...
const main = effOptChain(randomNum) (x =>
  effOptChain(randomNum) (y =>
    x + y >= 100
      ? effOf(None)
      : Eff(() => Some([x, y]))));

// no effect is performed yet (B)

main.eff; // logs "processing..." and either None or e.g. Some([25, 50])
```
[run code](https://repl.it/@scriptum/FelineLoyalCache)

The `OptionT`/`Eff` monad transformer stack is a type that either performs a computation and yields a result or does nothing. The special feature here is that it decouples the declaration of a value from the actual creation of this value by deferring this decision. It is up to the consumer not the producer of this type whether it actually runs the computation. Given this we can compose values of this type in a purely functional manner without actually performing any computational effects. Whe can observe this behavior in line `B`. Upt ot this point there is no effect. Only when we access `main.eff` we enforce evaluation and the logging side effect in line `A` is performed.
