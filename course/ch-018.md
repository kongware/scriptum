...
const match = (tx, o) =>
  tx.tag in o
    ? o[tx.tag] (tx)
    : _throw(new TypeError(`unknown property "${tx.tag}" in tagged union`));
...
const mmx = Array(10)
  .fill(0)
  .map((_, i) => Some(randNum()));

const main =
  optArrChainT(mmx) (x =>
    x === 0 || x >= 8
      ? [None]
      : optArrOfT("f" + "o".repeat(x)));

main; // array of random strings, e.g. [Some("foooo"), Some("fo"), None, Some("foooooooo"), ...]
```
[run cdoe](https://repl.it/@scriptum/PaleStrictTraining)

The combined effect is a non-deterministic array of ten optional strings. The lenght of the strings depends on a random number between 0 and 10. If null or a number greater than eight is produced, a `None` value is stored in the array. Otherwise the length of the string depends on the random number. I could have dropped the `None` values by utilizing the non-deterministic effect of the array type but diceded to keep them to emphasize the optional effect (line `A`).
...
#### `OptionT`/`EitherT`/`Array`

* add short circuit semantics to the transformer stack

#### `OptionT`/`EitherT`/`EffT`/`Array`

* add non-strictness to the transformer stack

#### `OptionT`/`EitherT`/`EffT`/`ArrayT`/`Trampoline`
