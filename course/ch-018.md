...
main.eff; // either None or e.g. Some([25, 50])
```
[run code](https://repl.it/@scriptum/FelineLoyalCache)

The `OptionT`/`Eff` monad transformer stack is a type that either performs another effect or does nothing. However, the special characteristic of this transformer stack is that it decouples value declaration from value creation. Given this we can compose such computations in a purely functional manner without actually performing the effects.
