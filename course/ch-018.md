...
The array contains individual elements that might have a value (`Some(x)`) or not (`None`). In the latter case values are omitted. Other monads, however, do not compose at all or yield unexpected results. Let us rearrange the order of both monads to demonstrate this:
...
At first sight the yielded values might seem appropriate, but `main3` actually exhibits an unexpected behavior. The semantics of the `Option` type is quite simple: As soon as a single `None` value is yielded the entire computaion is short circuited and returns `None`. While `main3` is the result of two arguments `mmx`/`mmz` of type `Some` the action `fmm` produces a `None` as soon as it reaches the `0` value within `mmz`. Consequenlty `main3` is meant to yield `None`. This unexpected behavior is equivalent to an error. The monad instances of `Option`/`Array` do not compose in a mechanical way.
