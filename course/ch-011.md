...
It is crucial to understand that immutability in a multi-paradigm language is not a silver bullet. You must use data types in a way that they were designed to be used. Do not attempt to convert mutable data types into immutable ones.
...
```
The mutation only takes place inside `take` and is not visible in the parent scope. Such mutations are manageable, because race conditions and other potential errors are now delimited to the function scope.
