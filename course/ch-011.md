...
Often related data is not available available at once but grows durng the course of the application. Instead of mutating a data structure with new components of data you can only loosely couple it by sharing the structure. Later it is possible to recover data relationships by comparing this very structure. Here is a rather contrieved example:
...
```
Looking at these data structures you can easily deduce compound data from its position within the underlying structure: `[1, "a", true]`, `[2, "b", false]` and `[3, "c", false]`. While this form of implicit data association works, it increases the mental load for the programmer the more complex the data structures evolve.
