...
It gets even harder. Several applicative computations must not necesserily form a sequence, but can also run in parallel, provided there is no inter-value or inter-effect dependency. In this case each applicative represents an independent computation. The evaluation order of composed applicative computations depends on the specific instance. It can either have no order at all, because it runs in parallel, it can run in sequence or in whatever order the particular instance determines. That means the evaluation order of applicative operations is not always deterministic.
