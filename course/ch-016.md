...
Applicative does not rely on the result value of the previous applicative operation and thus do not depend on a specific evaluation order. An applicative computation can either be evaluated in-sequence, in-parallel or in whatever order a particular instance determines. Every applicative represents an independent computation. 
