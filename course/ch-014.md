...
```
#### No escape of the functor

A functor must preserve the structure. From another perspective we can also say that it is not possible to escape the effectful context, at least not by means of the functor. You can define a getter or fold to retrieve the value inside the functorial context, of course. But this would be beyond the functor API and, more importantly, you could not generalize such a getter or fold, so that it would apply to any functor.

This means when you have a value inside an effectful context, it is stuck in this context for the rest of your program. Since the very purpose of functors is to apply pure functions inside effectful contexts instead of retrieving the values, this behavior is actually desired. Otherwise the `Functor` type class would be far less general.
