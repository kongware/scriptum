...
* time renders functions non-idempotent (`f(x) ≠ f(f(x))`)
* time renders functions non-associative (`f(x) (f(y) (z)) ≠ f(f(x) (y)) (z))`)
* time renders functions non-commutative (`f(x) (y) ≠ f(y) (x)`)

Losing this properties has quite practical implications on your functional programming experience. Side effects hamper
