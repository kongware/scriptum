...
It is rather difficult to talk about something as fundamental and general as state. This chapter is the attempt to give some guidance. Please note that I use the terms _state_ and _data_ for convenience to distinguish _mutual state_ from _immutable state_.
...
If we keep formalizing this pattern and add a couple of useful combinators, we will wind up with the state monad, which abstracts from the state threading. You do not need the state monad to mimic stateful compositions in functional programming, but it facilitates their construction and handling. There are other state related techniques like `IORef` or software transactional memory (STM). We will cover them in other chapters if this course.
