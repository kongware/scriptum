...
Purely functional programming only consists of expressions. An expression is a first class code unit that can be evaluated to a single value according to its particular rules of precedence. An  expression can be passed to and be returned by functions like any other ordinary value.
