...
Applicative does not rely on the result of the previous applicative computation. As a result such computations do not necessarily have a notion of order. Combining two applicative contexts therefore means to perform the effects either in sequence or in parallel. The important fact is that both effects have to take place exactly once. So applicative combining means either to sequence or to parallelize.
