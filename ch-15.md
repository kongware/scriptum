...
optAp(optMap(add) (Some(2))) (Some(3)); // Some(5)
optAp(optMap(add) (None)) (Some(3)); // None
```
[run code](https://repl.it/repls/CyanCuddlyOrganization)

When the computation comes upon the `None` case, no matter at which position, the whole applicative composition is short circuited and the pure function (namely `add`) is never evaluated.
