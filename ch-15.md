...
The crucial aspect of applicatives is that when the contexts are combined, the effect each context have on its value(s) is performed exactly ONCE, without exception. Let us illustrate this mechanism with the single linked list from the previous section:
...
comp(arrAp) (arrMap(add)) ([1, 2, 3]) ([0, 20, 300]);
```
