...
We have seen that the `Applicative` properties operate on contexts. `ap` lifts an n-ary functions `f` into `n` contexts and applies it within the same context to the contained values. `f` can choose the method of combining, whether it accumulates, aggregates or picks values.
