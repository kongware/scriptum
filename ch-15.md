...
We have seen that the `Applicative` type class properties solely operate on context. `of` creates a minimal context and puts whatever value into it. `ap` lifts an n-ary function `f` into `n` contexts and applies it to the contained values. Being applied by `ap` `f` can freely choose the method it is going to combine the involved values with. It can accumulate, aggregate or pick them. The context itself, however, is completely opaque to `f`. Everything takes place within the same context `f` is nt aware of.

### The two notions of values


The way a value is created, whether there is a value at all or even many values, how values are arranged is determined by the context. It is the effect the context has on its value(s).
