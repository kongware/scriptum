...
Since applicatives are functors they compose, i.e. we can compose contexts and their effects. I want to take the chance and demonstrate on the follwoing a bit more complex example the process of abstracting in functional programming. We start with the most detailed encoding and then try to abstract from boilerplate and common patterns.
...
```
This is the applicative operation to handle an async computation that may fail or yields any number of results. We canget rid of the lambdas by using point-free style:
...
As you can see we are able to combine two composed contexts/effetcs with `main`/`main2` and everything works as expected.
