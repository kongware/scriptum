...
The pure function `f` on the other hand is not context aware at all. The context it is being applied to values in is completely opaque to `f`. The pure function is object to this context and cannot change any property that belongs to the latter. From the perspective of the context `f`'s application is completely mechanic and deterministic.
