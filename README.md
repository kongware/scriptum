...
A type-directed yet untyped functional library that adapts well-known functional patterns to Javascript.
...
If an expression is shared across different points in our code we assign it to a name so that it is only calculated once. Since there are only `let` statements in Javascript scriptum mimics the expression counterpart with the `_let` combinator, which merely utilizes Javascript's default parameters in a creative way: `const _let = f => f();`. Besides we use such let bindings if we want to avoid repitition.
...
Persistent data structures are immutable and yet efficient by utilizing structural sharing.
...
```Javascript
const o = Hashed(Lazy(() => "hash1")) ({foo: 1});
const p = Hashed(Lazy(() => "hash2")) ({bar: 2});
const q = Hashed(Lazy(() => "hash3")) ({baz: 3});

const h1 = hamtSet(o, "foo") (hamtRoot);
const h2 = hamtSet(p, "bar") (h1);
const h3 = hamtSet(q, "baz") (h2);

hamtGet(o) (h3); // "foo"
hamtGet(p) (h3); // "bar"
hamtGet(q) (h3); // "baz"
```
Instead of strings like `"hash"` you would call a hash function inside the lazy wrapper, of course.
